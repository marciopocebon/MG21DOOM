...
__attribute__ ((section(".ramfunc"), noinline)) void flashEraseCurrentPage()
{
...
__attribute__ ((section(".ramfunc"), noinline)) void flashProgramCurrentWord(uint32_t word)
{
    MSC->WDATA = word;
    while (!(MSC->IF & MSC_IF_WRITE));    // wait until write operation is done
}
