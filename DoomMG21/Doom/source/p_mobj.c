...
void P_XYMovement(mobj_t *mo)
{
     ...
     */
    if ((mo->flags & MF_CORPSE ) && (mo->momx > FRACUNIT / 4 || mo->momx < -FRACUNIT / 4 || mo->momy > FRACUNIT / 4 || mo->momy < -FRACUNIT / 4) && mo->floorz != getMobjSubesctor(mo)->sector->floorheight)
...
void P_NightmareRespawn(mobj_t *mobj)
{
    ...
    mo = P_SpawnMobj(mobj->x, mobj->y, getMobjSubesctor(mobj)->sector->floorheight, MT_TFOG);
...
mobj_t* P_SpawnMobj(fixed_t x, fixed_t y, fixed_t z, mobjtype_t type)
{
    ...
    mobj->floorz = getMobjSubesctor(mobj)->sector->floorheight;
    mobj->ceilingz = getMobjSubesctor(mobj)->sector->ceilingheight;
...
//   mobj->z = z == ONFLOORZ ? mobj->floorz : z == ONCEILINGZ ?    mobj->ceilingz - getMobjHeight(mobj) : z;
    mobj->z =
            z == ONFLOORZ ? getMobjSubesctor((mobj_t*) mobj)->sector->floorheight :
            z == ONCEILINGZ ? getMobjSubesctor((mobj_t*) mobj)->sector->ceilingheight - getMobjHeight((mobj_t*) mobj) : z;
...
void P_RemoveMobj(mobj_t *mobj)
{
    ...
    {
        P_RemoveStaticMobj((static_mobj_t*) mobj);
        return;
    }
