 ...
 *             stack in almost all the function calls here, but it is huge (15k)
...
#define SAVEGAME_ADDRESS (spiFlashGetSize() - 4096)
#define SETTINGS_ADDRESS (spiFlashGetSize() - 2 * 4096)
...
void G_UpdateSaveGameStrings()
{
    ...
    byte *loadbuffer = (byte*) displayBuffer;
    ...
    for (int i = 0; i < 8; i++)
    {
        ...
        else
        {
            if (_g->gamemode == commercial)
            {
                strcpy(savegamestrings[i], "MAP ");

                saveslots[i].gamemap = str2num(&savegamestrings[i][4]);
            }
            else
            {
...
void writeToExternalFlash(uint32_t address, void *buffer, int size)
{
    ...
    //
    spiFlashProgramForDualRead(address, buffer, size);
...
static void G_DoSaveGame(boolean menu)
{
    ...
    // Therefore we can "safely" use it as a temporary work buffer.
    // Yes, this is a bad hack, but it works :)
    byte *savebuffer = (byte*) displayBuffer;
    // read old data
    spiFlashSetAddress(SAVEGAME_ADDRESS);
...
void G_LoadSettings()
{
    ...
    if (settings.cookie == settings_cookie)
    {
        ...
        _g->snd_SfxVolume =
                (settings.soundVolume > 15) ? 15 : settings.soundVolume;
        _g->snd_MusicVolume =
                (settings.musicVolume > 15) ? 15 : settings.musicVolume;
