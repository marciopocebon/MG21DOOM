...
{
    static const event_t st_notify =
...
boolean AM_Responder(event_t *ev)
{
    ...
    else if (ev->type == ev_keydown)
    {
        ...
        else if (ch == key_map_follow && _g->gamekeydown[key_use])
        {
            ...
            _g->player.message =
                    (_g->automapmode & am_follow) ? AMSTR_FOLLOWON : AMSTR_FOLLOWOFF;
        }                                                         //    |
...
static void AM_drawWalls(void)
{
    ...
    for (i = 0; i < _g->numlines; i++)
    {
        ...
        if (_g->linedata[i].r_flags & RF_MAPPED) // 2020-03-14 next-hack was ML_MAPPED, changed to RF_MAPPED
        {
            ...
            (mapcolor_exit && (line_special == 11 || line_special == 52 || line_special == 197 || line_special == 51 || line_special == 124 || line_special == 198))
            {
            ...
            else /* now for 2S lines */
            {
                ...
                if (mapcolor_tele && !(_g->lines[i].flags & ML_SECRET) && (line_special == 39 || line_special == 97 || line_special == 125 || line_special == 126))
                { // teleporters
                ...
                (map_secret_after && ((P_WasSecret(frontsector) && !P_IsSecret(frontsector)) || (P_WasSecret(backsector) && !P_IsSecret(backsector)))) || //jff 3/9/98 add logic to not show secret til after entered
        ...
        else if (_g->player.powers[pw_allmap]) // computermap visible lines
        {
            ...
            {
                if (mapcolor_flat || !backsector || backsector->floorheight != frontsector->floorheight || backsector->ceilingheight != frontsector->ceilingheight)
...
static void AM_drawPlayers(void)
{
    ...
    NUMPLYRLINES, 0, _g->player.mo->angle, mapcolor_sngl,      //jff color
