...
{
    _g->validcount++;
    P_RecursiveSound(getMobjSubesctor(emitter)->sector, 0, target);
}
...
{
    sector_t *sec = getMobjSubesctor((mobj_t*) actor)->sector;
...
static boolean P_SmartMove(mobj_t *actor)
{
    ...
    on_lift = target && target->health > 0 && getMobjSubesctor(target)->sector->tag == getMobjSubesctor(actor)->sector->tag && P_IsOnLift(actor);
...
{
    mobj_t *targ = getSectorSoundTarget(getMobjSubesctor(actor)->sector);
...
// compatibility mode, we remove the limit.                         // phares
            ...
            //   V
            {
                ...
                if ((newmobj->z > (getMobjSubesctor(newmobj)->sector->ceilingheight - getMobjHeight(newmobj))) || (newmobj->z < getMobjSubesctor(newmobj)->sector->floorheight))
                {
                    // kill it immediately
